...
class UserExamManagement extends StatelessWidget {
  ...
  Future<void> _showPaperDialog(BuildContext context, {ExamPaper? paper}) async {
    final isEdit = paper != null;
    final titleCtrl = TextEditingController(text: isEdit ? paper.title : '');
    final durCtrl = TextEditingController(text: isEdit ? '${paper.durationMinutes}' : '30');
    bool saving = false;
    String? titleForNav;
    ...
    final newExamId = await showDialog<String>(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setD) => AlertDialog(
        ...
        title: Text(isEdit ? 'Sửa Đề Thi' : 'Tạo Đề Thi Mới',
            style: const TextStyle(color: AppColors.primary, fontWeight: FontWeight.bold)),
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(controller: titleCtrl,
              decoration: InputDecoration(labelText: 'Tiêu đề đề thi',
                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
          const SizedBox(height: 12),
          TextField(controller: durCtrl, keyboardType: TextInputType.number,
              decoration: InputDecoration(labelText: 'Thời gian làm bài (phút)',
                  border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
        ]),
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: saving ? null : () async {
              ...
              try {
                ...
                final creatorName = (userDoc.data()?['name'] as String?) ?? user.email ?? 'Ẩn danh';
                if (isEdit) {
                  await FirestoreService.updateExamPaper(paper.id, {
                    'title': titleCtrl.text.trim(),
                    'duration_minutes': int.tryParse(durCtrl.text) ?? 30,
                  });
                  ...
                  final data = {
                    'title': titleCtrl.text.trim(),
                    'duration_minutes': int.tryParse(durCtrl.text) ?? 30,
                    'creatorId': user.uid, 'creatorName': creatorName, 'isPublished': false,
                  };
                  titleForNav = titleCtrl.text.trim();
                  final ref = await FirestoreService.createExamPaper(data);
            ...
            child: saving
                ...
                : Text(isEdit ? 'Lưu' : 'Tạo & Thêm câu hỏi', style: const TextStyle(color: Colors.white)),
          ),
        ],
      )),
    );
    ...
    if (newExamId != null && context.mounted) {
      Navigator.push(context, MaterialPageRoute(
        builder: (_) => UserExamQuestionsScreen(
          examPaperId: newExamId, examTitle: titleForNav ?? '', allowStartExam: true,
        ),
      ));
    }
  }
      ...
      actions: [
        ...
        ElevatedButton(
          ...
          onPressed: () async {
            ...
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Đã xóa!'), backgroundColor: Colors.red));
            }
          },
  ...
  Future<void> _togglePublish(BuildContext context, ExamPaper p) async {
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(p.isPublished ? '📝 Đã chuyển về nháp' : '✅ Đã xuất bản bài thi!'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Bài thi của tôi'),
          backgroundColor: AppColors.primary, foregroundColor: Colors.white),
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () => _showPaperDialog(context),
        backgroundColor: AppColors.accent,
        icon: const Icon(Icons.add_rounded, color: Colors.white),
        label: const Text('Tạo đề thi', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
      ),
        ...
        builder: (context, snap) {
          ...
          if (!snap.hasData || snap.data!.docs.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              const Text('Bạn chưa tạo đề thi nào.', style: TextStyle(color: Colors.grey, fontSize: 16)),
              const SizedBox(height: 12),
              ElevatedButton.icon(
                onPressed: () => _showPaperDialog(context),
                style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary,
                ...
                label: const Text('Tạo đề đầu tiên', style: TextStyle(color: Colors.white)),
              ),
            ]));
          }
                ...
                child: ListTile(
                  ...
                  leading: Container(width: 48, height: 48,
                    decoration: BoxDecoration(
                      gradient: LinearGradient(colors: p.isPublished ? AppColors.primaryGradient : [Colors.grey, Colors.blueGrey]),
                      borderRadius: BorderRadius.circular(12)),
                    child: const Icon(Icons.assignment_rounded, color: Colors.white, size: 24)),
                  title: Row(children: [
                    Expanded(child: Text(p.title, style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.primary))),
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                      decoration: BoxDecoration(
                        color: p.isPublished ? Colors.green.shade50 : Colors.orange.shade50,
                        borderRadius: BorderRadius.circular(6)),
                      child: Text(p.isPublished ? 'Đã xuất bản' : 'Nháp',
                        style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold,
                          color: p.isPublished ? Colors.green : Colors.orange)),
                    ),
                  ]),
                  subtitle: Text('⏱ ${p.durationMinutes} phút', style: const TextStyle(color: AppColors.textSecondary)),
                  trailing: PopupMenuButton<String>(
                    onSelected: (v) {
                      if (v == 'questions') {
                        Navigator.push(context, MaterialPageRoute(
                            builder: (_) => UserExamQuestionsScreen(examPaperId: p.id, examTitle: p.title)));
                      }
                      if (v == 'edit') _showPaperDialog(context, paper: p);
                      if (v == 'publish') _togglePublish(context, p);
                      if (v == 'share') {
                        Clipboard.setData(ClipboardData(text: 'https://vku-vietnamese-learning.web.app/?examId=${p.id}'));
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('🔗 Đã sao chép link!'), backgroundColor: Colors.green));
                      }
                      if (v == 'delete') _confirmDelete(context, p);
                    },
                    itemBuilder: (_) => [
                      const PopupMenuItem(value: 'questions', child: Row(children: [Icon(Icons.list_alt, size: 18, color: AppColors.accent), SizedBox(width: 8), Text('Câu hỏi')])),
                      const PopupMenuItem(value: 'edit', child: Row(children: [Icon(Icons.edit, size: 18, color: Colors.blue), SizedBox(width: 8), Text('Sửa')])),
                      PopupMenuItem(value: 'publish', child: Row(children: [
                        Icon(p.isPublished ? Icons.unpublished : Icons.publish, size: 18, color: p.isPublished ? Colors.orange : Colors.green),
                        const SizedBox(width: 8),
                        Text(p.isPublished ? 'Bỏ xuất bản' : 'Xuất bản'),
                      ])),
                      const PopupMenuItem(value: 'share', child: Row(children: [Icon(Icons.share, size: 18, color: AppColors.primary), SizedBox(width: 8), Text('Chia sẻ')])),
...
class UserExamQuestionsScreen extends StatefulWidget {
  ...
  const UserExamQuestionsScreen({
    super.key, required this.examPaperId, required this.examTitle,
    this.allowStartExam = false,
  });

  @override
  State<UserExamQuestionsScreen> createState() => _UserExamQuestionsScreenState();
}
...
class _UserExamQuestionsScreenState extends State<UserExamQuestionsScreen> {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setD) {
        ...
        Future<void> save() async {
          ...
          if (target.isEmpty) {
            ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Vui lòng nhập nội dung!'), backgroundColor: Colors.orange));
            return;
          }
          ...
          setD(() => saving = true);
          try {
            final opts = selType == ExamQuestionType.mcq
                ? optCtrls.map((c) => c.text.trim()).where((t) => t.isNotEmpty).toList()
                : <String>[];
            int orderIndex = isEdit ? q.orderIndex : 0;
            if (!isEdit) {
              try {
                final existing = await FirebaseFirestore.instance
                    .collection('exam_questions').where('examPaperId', isEqualTo: widget.examPaperId).get();
            ...
            if (context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                content: Text(isEdit ? '✅ Đã cập nhật!' : '✅ Đã thêm câu hỏi!'),
          ...
          content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              _typeChip(setD, 'Phát âm', ExamQuestionType.pronunciation, selType, AppColors.primary,
                  () => setD(() => selType = ExamQuestionType.pronunciation)),
            ]),
            ...
            if (selType == ExamQuestionType.mcq) ...[
              const SizedBox(height: 14),
              const Text('Các lựa chọn:', style: TextStyle(fontWeight: FontWeight.bold, color: AppColors.primary)),
              const SizedBox(height: 8),
              ...List.generate(4, (i) => Padding(
                  padding: const EdgeInsets.only(bottom: 8),
                  child: TextField(controller: optCtrls[i],
            ...
            if (selType == ExamQuestionType.pronunciation) ...[
              ...
              Container(
                ...
                child: const Row(children: [
                  ...
                  SizedBox(width: 8),
                  Expanded(child: Text('AI sẽ tự động chấm điểm phát âm.\nĐẠT khi accuracy ≥ 80%.', style: TextStyle(fontSize: 12, color: AppColors.primary))),
                ]),
              ),
            ],
          ])),
  ...
  Widget _typeChip(StateSetter setD, String label, ExamQuestionType type, ExamQuestionType current, Color activeColor, VoidCallback onTap) {
    ...
    return Expanded(child: GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(color: isActive ? activeColor : Colors.grey[100], borderRadius: BorderRadius.circular(10)),
        child: Center(child: Text(type == ExamQuestionType.mcq ? '🎧 $label' : '🎙 $label',
            style: TextStyle(fontWeight: FontWeight.bold, color: isActive ? Colors.white : Colors.grey[700]))),
      ),
    ));
  }
  ...
  Future<void> _startExam(BuildContext context) async {
    try {
      final snap = await FirestoreService.examQuestionsStream(widget.examPaperId).first;
      final questions = FirestoreService.parseExamQuestions(snap);
      if (!context.mounted) return;
      if (questions.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('⚠️ Chưa có câu hỏi nào!'), backgroundColor: Colors.orange));
        return;
      }
      final paper = await FirestoreService.getExamPaperById(widget.examPaperId);
      if (!context.mounted || paper == null) return;
      Navigator.push(context, MaterialPageRoute(builder: (_) => NewExamScreen(paper: paper, questions: questions)));
    } catch (e) {
        ...
        actions: [
          if (widget.allowStartExam)
            TextButton.icon(
              ...
              label: const Text('Thi thử', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        ...
        builder: (context, snap) {
          ...
          if (!snap.hasData || snap.data!.docs.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              const SizedBox(height: 16),
              const Text('Chưa có câu hỏi nào.', style: TextStyle(color: Colors.grey, fontSize: 16)),
            ]));
          }
              ...
              return Container(
                ...
                child: Row(children: [
                  Container(width: 36, height: 36,
                    decoration: BoxDecoration(color: (isPron ? AppColors.primary : AppColors.accent).withValues(alpha: 0.12), borderRadius: BorderRadius.circular(8)),
                  ...
                  PopupMenuButton<String>(
                    ...
                    itemBuilder: (_) => const [
                      PopupMenuItem(value: 'edit', child: Row(children: [Icon(Icons.edit, size: 18, color: Colors.blue), SizedBox(width: 8), Text('Sửa')])),
                      PopupMenuItem(value: 'delete', child: Row(children: [Icon(Icons.delete, size: 18, color: Colors.red), SizedBox(width: 8), Text('Xóa', style: TextStyle(color: Colors.red))])),
                    ],
                    icon: const Icon(Icons.more_vert, color: Colors.grey),
                  ),
                ]),
