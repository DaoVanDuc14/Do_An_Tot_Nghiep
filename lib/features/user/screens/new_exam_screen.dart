...
class NewExamScreen extends StatefulWidget {
  ...
  const NewExamScreen({super.key, required this.paper, required this.questions});
...
class _NewExamScreenState extends State<NewExamScreen> {
  ...
  void _confirmSubmit() {
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (unanswered > 0) Text('⚠️ Chưa làm: $unanswered', style: const TextStyle(color: Colors.orange, fontWeight: FontWeight.bold)),
        if (unanswered == 0) const Text('✅ Đã hoàn thành tất cả!', style: TextStyle(color: Colors.green)),
      ]),
      actions: [
    ...
    body: Center(child: Padding(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Container(width: 100, height: 100,
          decoration: const BoxDecoration(gradient: LinearGradient(colors: AppColors.primaryGradient), shape: BoxShape.circle),
          child: const Icon(Icons.assignment_rounded, color: Colors.white, size: 52)),
        const SizedBox(height: 24),
        Text(widget.paper.title, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.primary), textAlign: TextAlign.center),
        const SizedBox(height: 24),
        _infoRow(Icons.help_outline, '${widget.questions.length} câu hỏi'),
        const SizedBox(height: 10),
        _infoRow(Icons.timer_outlined, '${widget.paper.durationMinutes} phút'),
        const SizedBox(height: 10),
        _infoRow(Icons.star_outline, '10 điểm / câu đúng'),
        const SizedBox(height: 10),
        _infoRow(Icons.mic_rounded, 'Phát âm: thu âm & chấm AI (≥80% = đạt)'),
  ...
  Widget _infoRow(IconData icon, String text) => Row(mainAxisAlignment: MainAxisAlignment.center, children: [
    ...
    Flexible(child: Text(text, style: const TextStyle(fontSize: 15, color: AppColors.textSecondary))),
  ]);
  ...
  Widget _buildExam() {
    ...
    final answered = _answers.length;
    final total = widget.questions.length;
    final timerColor = _remaining < 60 ? Colors.red : (_remaining < 300 ? Colors.orange : Colors.white);
          ...
          child: Row(children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: (_remaining < 60 ? Colors.red : Colors.white).withValues(alpha: _remaining < 60 ? 0.2 : 0.15),
                borderRadius: BorderRadius.circular(20),
              ),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.timer_outlined, color: timerColor, size: 18),
                const SizedBox(width: 4),
                Text(_fmt(_remaining), style: TextStyle(color: timerColor, fontWeight: FontWeight.bold, fontSize: 16)),
              ]),
            ),
            const Spacer(),
            // Tên bài thi
            Flexible(child: Text(widget.paper.title, style: const TextStyle(fontSize: 14), overflow: TextOverflow.ellipsis)),
            const Spacer(),
            // Nút nộp bài
            TextButton(
              ...
              return Padding(
                ...
                child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    child: Center(child: Text('${i + 1}', style: TextStyle(color: fg, fontWeight: FontWeight.bold, fontSize: 14))),
                  ),
                ),
              );
            })),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            Container(
              ...
              child: Column(children: [
                ...
                // Số câu
                Text('Câu ${_idx + 1} / $total', style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),
                const SizedBox(height: 10),
                ...
                if (isMcq) ...[
                  ...
                  Container(
                    ...
                    child: Column(children: [
                      const Text('Hãy đọc câu sau:', style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
                      const SizedBox(height: 8),
                      Text(q.targetText, textAlign: TextAlign.center,
                        style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: AppColors.primary, height: 1.4)),
                    ]),
                  ),
                ],
              ]),
            ...
            if (!isMcq) ...[
              PronunciationRecorderWidget(
                ...
                recordId: q.id,
                initialResult: _answers.containsKey(q.id) && _answers[q.id]!['type'] == 'pronunciation'
                    ? PronunciationResult(
                        targetText: q.targetText,
                        recognizedText: '',
                        accuracy: _answers[q.id]!['accuracy'] as int? ?? 0,
                        passed: _answers[q.id]!['isCorrect'] as bool? ?? false,
                      )
                    : null,
      ...
      bottomNavigationBar: Container(
        ...
        child: Row(children: [
          ...
          Expanded(child: ElevatedButton.icon(
            onPressed: _idx < total - 1 ? () => setState(() => _idx++) : _confirmSubmit,
            style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 14),
                backgroundColor: AppColors.primary, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
            icon: Icon(_idx < total - 1 ? Icons.arrow_forward_ios_rounded : Icons.check_rounded, size: 16),
            label: Text(_idx < total - 1 ? 'Tiếp theo' : 'Nộp bài', style: const TextStyle(color: Colors.white)),
          )),
        ]),
      ),
    );
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text('Đã làm: ${_answers.length}/$total', style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),
          const SizedBox(height: 16),
          GridView.builder(
            ...
            itemBuilder: (_, i) {
              ...
              final isCurrent = i == _idx;
              Color bg;
              if (isCurrent) { bg = AppColors.primary; }
              else if (isAnswered) { bg = AppColors.accent; }
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  child: Center(child: Text('${i + 1}', style: TextStyle(
                      color: isCurrent || isAnswered ? Colors.white : Colors.grey[700], fontWeight: FontWeight.bold))),
                ),
              );
            },
          ),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            _legend(AppColors.primary, 'Đang làm'), const SizedBox(width: 16),
            _legend(AppColors.accent, 'Đã làm'), const SizedBox(width: 16),
